...
app.factory('deviceState', function($resource, CONFIG) {
  ...
  return  $resource('http://'+CONFIG.homey_ip + '/api/device/:deviceId', {}, {
    'get': {
    	action: 'GET',
    	isArray: false,
      headers:{"Authorization": "Bearer " + CONFIG.homey_api, "Content-Type": "application/json"}
...
app.controller('appController', function ($scope, $uibModal, $localStorage, $timeout, CONFIG, allDevices, deviceState, allZones, hmTouchEvents, $http) {
  ...
  $scope.open = function (size) {
    ...
    $scope.tap = function(currentId, cmd){
      ...
      $http.put('http://'+CONFIG.homey_ip+'/api/device/'+currentId, {'onoff':cmd}, {headers: {"Authorization": "Bearer " + CONFIG.homey_api, "Content-Type": "application/json"}});

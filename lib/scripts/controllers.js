...
app.factory('setDevice', function($http, CONFIG) {
  var obj = {};
      ...
      obj.dim = function(currentId, dimValue){
        return $http.put('http://'+CONFIG.homey_ip+'/api/device/'+currentId, {'dim':dimValue}, CONFIG.httpconfig);
      }
   return obj;
});
...
app.controller('mainController', function ($scope, $uibModal, $localStorage, $timeout, CONFIG, allDevices, allZones, setDevice, hmTouchEvents) {
...
// Opens the device dimmer slider
  $scope.openDeviceSettings = function (thisdevice) {
    ...
    $scope.press = function(thisdevice){
      //setDevice.dim(currentId, dimValue);

...
app.factory('allDevices', function($http, CONFIG, $localStorage, $q) {
  var obj = {};
      obj = function(zoneId){
        return $http.get('https://'+CONFIG.homey_ip + '/api/manager/devices/device/?zone='+zoneId, CONFIG.httpconfig)
}
...
app.factory('setDevice', function($http, CONFIG) {
  var obj = {};
      obj.onoff = function(currentId, cmd){
          return $http.put('https://'+CONFIG.homey_ip+'/api/manager/devices/device/'+currentId+'/state', {'onoff':cmd}, CONFIG.httpconfig);
      }
      obj.dim = function(currentId, dimValue){
        return $http.put('https://'+CONFIG.homey_ip+'/api/manager/devices/device/'+currentId+'/state', {'dim':dimValue}, CONFIG.httpconfig);
      }
   return obj;
});

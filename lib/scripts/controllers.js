...
app.factory('allDevices', function($http, CONFIG, $localStorage, $q) {
  var obj = {};
      obj = function(zoneId){
        promises = [];
        return $http.get('//'+CONFIG.homey_ip + '/api/manager/devices/?zone='+zoneId, CONFIG.httpconfig)
                .then(function (response) {
                    var devices = response.data.result;
                    return $q.all(devices.map(function (device) {
                        return $http.get('//'+CONFIG.homey_ip + '/api/device/'+device.id, CONFIG.httpconfig)
                                .then(function (response) {
...
app.factory('setDevice', function($http, CONFIG) {
  var obj = {};
      obj.onoff = function(currentId, cmd){
          return $http.put('//'+CONFIG.homey_ip+'/api/device/'+currentId, {'onoff':cmd}, CONFIG.httpconfig);
      }
      obj.dim = function(currentId, dimValue){
        return $http.put('//'+CONFIG.homey_ip+'/api/device/'+currentId, {'dim':dimValue}, CONFIG.httpconfig);
      }
   return obj;
});
...
app.factory('allZones', function($http, CONFIG) {
  var obj = {};
      obj = function(){
          return $http.get('//'+CONFIG.homey_ip + '/api/manager/zones/', CONFIG.httpconfig);
      }
   return obj;
});
...
app.factory('sendSpeech', function($http, CONFIG) {
  var obj = {};
      obj = function(cmd){
          return $http.post('//'+CONFIG.homey_ip + '/api/manager/speech-input/', {"transcript": cmd}, CONFIG.httpconfig);
      }
   return obj;
});
